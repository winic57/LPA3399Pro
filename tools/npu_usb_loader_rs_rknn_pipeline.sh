...
export FW_DIR=${USB_FW_DIR:-/usr/share/npu_fw}
...
export BOOT_ADDR=${BOOT_ADDR:-0x20000}
...
wait_for_usb3() {
  ...
  for i in $(seq 0 "$WAIT_USB3_TIMEOUT_SEC"); do
    ...
    if printf '%s\n' "$out" | grep -qi "$EXPECT_USB_ID"; then
      echo "$out" | grep -Ei '2207:|rockchip|rk3xxx' || true
      log "USB3 criterion met: $EXPECT_USB_ID"
      return 0
    fi
    sleep "$WAIT_USB3_POLL_SEC"
  done
  log "USB3 criterion NOT met: $EXPECT_USB_ID"
  return 1
}
